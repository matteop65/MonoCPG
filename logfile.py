...
def logtext(text,log_path="logfile.txt"):
    """
...
def logevent(event,level=1,log_path="logfile.txt",counter=None):
    """
        Event = message
    ...
    # 4 = FATAL
    # 5 = SUCCESS
    levelcode = ["DEBUG","INFO","WARNING","ERROR","FATAL", "SUCCESS"]
    ...
    f = open(log_path, 'a+')
    f.write(f'[{levelcode[level]}] {event}\n')
    f.close()
