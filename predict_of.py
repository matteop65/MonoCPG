...
def predict_keypoints_o(JSON):
    ...
    inference_dataset = KeyPointsDataset(inference_keys, train_aug, train=False)
    ...
    sample_val_images, sample_val_keypoints = next(iter(inference_dataset))
    print(f'type sample_val_images: {type(sample_val_images)}')
    ...
    predicted_keypnts_folder = f'data/results/predicted_keypnts'
    expand_user = os.path.expanduser('~/Documents/GitHub/MonoCPG')
    new_folder = os.path.join(expand_user, predicted_keypnts_folder)
    ...
    for idx, num in enumerate(predictions):
        ...
        img_path = f'{new_folder}/{os.path.splitext(img_name)[0]}.txt'
        # print(img_path)
...
if __name__ == "__main__":
    predict_keypoints('data/results/annotations.json')
